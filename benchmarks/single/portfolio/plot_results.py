"""Plot portfolio-optimisation benchmark results from a JSON produced by
``benchmark_portfolio.py``.

The JSON is a flat list of cell dicts with keys:
  n, k, k_n_ratio, N, solver_name, device, finite, mean_ms, std_ms, ...

Usage:
    python plot_results.py
    python plot_results.py --in results/benchmark_portfolio.json \\
                           --out results/benchmark_portfolio.pdf
"""
...
def plot_results(results, output_path):
    """Solve time vs. n (number of assets), one line per (k/n ratio, solver)."""
    ...
    for r in results:
        if not r.get('finite', False):
            continue
        key = (r.get('k_n_ratio', 0.1), r['solver_name'])
        series.setdefault(
            key,
            {'n': [], 'mean': [], 'std': [], 'device': r.get('device', '?')},
        )
        series[key]['n'].append(r['n'])
        series[key]['mean'].append(r['mean_ms'])
        series[key]['std'].append(r['std_ms'])

    ratios = sorted({r.get('k_n_ratio', 0.1) for r in results
    ...
    for (ratio, name), data in sorted(series.items(), key=_draw_key, reverse=True):
        ...
        ns_vals = np.array(data['n'])[order]
        means = np.array(data['mean'])[order]
        stds = np.array(data['std'])[order]

        label = name if len(ratios) == 1 else f'{name}, $k/n={ratio:g}$'
        ...
        if np.any(stds > 0):
            ax.fill_between(ns_vals, means - stds, means + stds,
                            color=color, alpha=0.12, linewidth=0)
        plotted_handles[(ratio, name)] = line

    ax.set_xscale('log')
    ax.set_yscale('log')
    ax.set_xlabel(r'$n$ (number of assets)')
    ax.set_ylabel(r'Solve time (ms)')
    ax.set_title(r'Portfolio Optimization QP Benchmark')
...
def main():
    parser = argparse.ArgumentParser(description='Plot portfolio benchmark.')
    parser.add_argument(
        '--in', dest='input_json', type=str,
        default=os.path.join(results_dir, 'benchmark_portfolio.json'),
        help='Path to the JSON produced by benchmark_portfolio.py.')
    parser.add_argument(
        '--out', dest='output_pdf', type=str,
        default=os.path.join(results_dir, 'benchmark_portfolio.pdf'),
