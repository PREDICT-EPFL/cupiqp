...
def create_init_guess_rhs_kernel(n: int, p: int,
    ...
    def init_guess_rhs_kernel(
        ...
        b_eq:   wp.array2d(dtype=wp.float64),  # type: ignore  (B, p)
        h_l:    wp.array2d(dtype=wp.float64),  # type: ignore  (B, m)
        h_u:    wp.array2d(dtype=wp.float64),  # type: ignore  (B, m)
        x_l:    wp.array2d(dtype=wp.float64),  # type: ignore  (B, n)
        x_u:    wp.array2d(dtype=wp.float64),  # type: ignore  (B, n)
        ...
        res_x:   wp.array2d(dtype=wp.float64), # type: ignore  (B, n)   out
        res_y:   wp.array2d(dtype=wp.float64), # type: ignore  (B, p)   out
        res_zl:  wp.array2d(dtype=wp.float64), # type: ignore  (B, num_hl) out
        res_zu:  wp.array2d(dtype=wp.float64), # type: ignore  (B, num_hu) out
        res_zbl: wp.array2d(dtype=wp.float64), # type: ignore  (B, num_xl) out
        res_zbu: wp.array2d(dtype=wp.float64), # type: ignore  (B, num_xu) out
    ):
        b, t = wp.tid()
...
def create_init_guess_project_to_central_path_kernel():
    ...
    @wp.func
    def project_z_to_central_path(c: wp.float64, mu: wp.float64) -> wp.float64:
        """Positive root of the central-path quadratic ``z*(z - c) = mu``.

            z = (c + sqrt(c**2 + 4*mu)) / 2
        """
        return (c + wp.sqrt(c * c + wp.float64(4.0) * mu)) * wp.float64(0.5)

    @wp.kernel
    def init_guess_project_to_central_path_kernel(
        delta_z: wp.array2d(dtype=wp.float64),   # type: ignore  (B, 1)
        mu:      wp.array(dtype=wp.float64),     # type: ignore  (B,)
        z_all:   wp.array2d(dtype=wp.float64),   # type: ignore  (B, num_ineq) in-out
        s_all:   wp.array2d(dtype=wp.float64),   # type: ignore  (B, num_ineq) out
    ):
        b, i = wp.tid()
