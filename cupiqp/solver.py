...
class SolverBase:
    ...
    def _stream_context(self):
        return torch.cuda.stream(self._stream) if self._stream is not None else nullcontext()
    ...
    def _setup_impl(self, P, c, A, b, G, h_u, h_l, x_u, x_l):
        ...
        self._work_z_1 = torch.empty(self._data.m, dtype=torch.float64, device='cuda')  # used to store intermediate results in _update_residuals_nr
        self._work_z_2 = torch.empty(self._data.m, dtype=torch.float64, device='cuda')  # used to store intermediate results in _update_residuals_nr

        self._work_z = torch.empty(self._data.num_hl + self._data.num_hu + self._data.num_xl + self._data.num_xu, dtype=torch.float64, device='cuda')  # used in _calculate_step to hold all concatenated slack or dual steps / results
        self._work_s = torch.empty(self._data.num_hl + self._data.num_hu + self._data.num_xl + self._data.num_xu, dtype=torch.float64, device='cuda')  # used in _calculate_step to hold all concatenated slack or dual steps / results
        self._work_primals = torch.empty(self._data.n, dtype=torch.float64, device='cuda')
        self._work_duals = torch.empty(self._data.p + self._data.num_hl + self._data.num_hu + self._data.num_xl + self._data.num_xu, dtype=torch.float64, device='cuda')  # used to hold the concatenated dual variables for computing the residuals in _update_residuals_nr
        self._work_residual = torch.empty((), dtype=torch.float64, device='cuda')
        self._work_reduce = torch.empty(8, dtype=torch.float64, device='cuda')  # used to hold the intermediate results of the reductions related to s_l, s_u, s_bl, s_bu and z_l, z_u, z_bl, z_bu
        ...
        self._work_primal_rel_norm = torch.empty(1, dtype=torch.float64, device='cuda')  # running max of primal relative norm terms
        self._work_dual_res_norm = torch.empty(1, dtype=torch.float64, device='cuda')    # running max of dual residual norm terms
        self._work_norm_temp = torch.empty(1, dtype=torch.float64, device='cuda')        # temp scalar for individual norm results

        self._mu_prev = torch.empty(1, dtype=torch.float64, device='cuda')
        self._mu_rate = torch.empty(1, dtype=torch.float64, device='cuda')

        # Pre-allocated constant for use inside CUDA graph capture (torch.tensor() is not allowed during capture)
        self._const_one = torch.ones(1, dtype=torch.float64, device='cuda')
        ...
        if pc is not None:
            ...
            self._unscale_primal_res_eq_factor = pc.delta_inv[n:n + p] if p > 0 else torch.empty(0, dtype=torch.float64, device='cuda')
            self._unscale_primal_res_ineq_hu = pc.delta_inv[n + p + self._data.idx_hu] if self._data.num_hu > 0 else torch.empty(0, dtype=torch.float64, device='cuda')
            self._unscale_primal_res_ineq_hl = pc.delta_inv[n + p + self._data.idx_hl] if self._data.num_hl > 0 else torch.empty(0, dtype=torch.float64, device='cuda')
            self._unscale_primal_res_b_xu = pc.delta_b_inv[self._data.idx_xu] if self._data.num_xu > 0 else torch.empty(0, dtype=torch.float64, device='cuda')
            self._unscale_primal_res_b_xl = pc.delta_b_inv[self._data.idx_xl] if self._data.num_xl > 0 else torch.empty(0, dtype=torch.float64, device='cuda')
            # Compute unscaled constraints RHS norm from original (pre-scaling) bounds
            self._constraints_rhs_inf_norm_unscaled = torch.zeros(1, dtype=torch.float64, device='cuda')
            if p > 0:
                torch.maximum(self._constraints_rhs_inf_norm_unscaled,
                           torch.max(torch.abs(pc.unscale_primal_res_eq(self._data.b))).unsqueeze(0),
                           out=self._constraints_rhs_inf_norm_unscaled)
            if self._data.num_hu > 0:
                torch.maximum(self._constraints_rhs_inf_norm_unscaled,
                           torch.max(torch.abs(pc.unscale_primal_res_ineq(self._data.h_u[self._data.idx_hu], self._data.idx_hu))).unsqueeze(0),
                           out=self._constraints_rhs_inf_norm_unscaled)
            if self._data.num_hl > 0:
                torch.maximum(self._constraints_rhs_inf_norm_unscaled,
                           torch.max(torch.abs(pc.unscale_primal_res_ineq(self._data.h_l[self._data.idx_hl], self._data.idx_hl))).unsqueeze(0),
                           out=self._constraints_rhs_inf_norm_unscaled)
            if self._data.num_xu > 0:
                torch.maximum(self._constraints_rhs_inf_norm_unscaled,
                           torch.max(torch.abs(pc.unscale_primal_res_b(self._data.x_u[self._data.idx_xu], self._data.idx_xu))).unsqueeze(0),
                           out=self._constraints_rhs_inf_norm_unscaled)
            if self._data.num_xl > 0:
                torch.maximum(self._constraints_rhs_inf_norm_unscaled,
                           torch.max(torch.abs(pc.unscale_primal_res_b(self._data.x_l[self._data.idx_xl], self._data.idx_xl))).unsqueeze(0),
                           out=self._constraints_rhs_inf_norm_unscaled)
        else:
            self._c_scaling_inv = torch.ones(1, dtype=torch.float64, device='cuda')
            self._unscale_dual_res_factor = torch.ones(n, dtype=torch.float64, device='cuda')
            self._unscale_primal_res_eq_factor = torch.ones(p, dtype=torch.float64, device='cuda') if p > 0 else torch.empty(0, dtype=torch.float64, device='cuda')
            self._unscale_primal_res_ineq_hu = torch.ones(self._data.num_hu, dtype=torch.float64, device='cuda') if self._data.num_hu > 0 else torch.empty(0, dtype=torch.float64, device='cuda')
            self._unscale_primal_res_ineq_hl = torch.ones(self._data.num_hl, dtype=torch.float64, device='cuda') if self._data.num_hl > 0 else torch.empty(0, dtype=torch.float64, device='cuda')
            self._unscale_primal_res_b_xu = torch.ones(self._data.num_xu, dtype=torch.float64, device='cuda') if self._data.num_xu > 0 else torch.empty(0, dtype=torch.float64, device='cuda')
            self._unscale_primal_res_b_xl = torch.ones(self._data.num_xl, dtype=torch.float64, device='cuda') if self._data.num_xl > 0 else torch.empty(0, dtype=torch.float64, device='cuda')
    ...
    def _solve_impl(self) -> Status:
        ...
        for iter in range(self.settings.max_iter):
            with nvtx.annotate(f"Solver::ipm_iteration"):
                ...
                if self._data.num_hl > 0 and bool(torch.any(self._result.z_l < epsilon)):
                    self._result.z_l += (self._result.z_l < epsilon) * epsilon
                    boundary_shifted = True
                if self._data.num_hu > 0 and bool(torch.any(self._result.z_u < epsilon)):
                    self._result.z_u += (self._result.z_u < epsilon) * epsilon
                    boundary_shifted = True
                if self._data.num_xl > 0 and bool(torch.min(self._result.z_bl) < epsilon):
                    self._result.z_bl += epsilon
                    boundary_shifted = True
                if self._data.num_xu > 0 and bool(torch.min(self._result.z_bu) < epsilon):
    ...
    def _initial_guess(self):
        ...
        self._result.x.fill_(0.0)
        self._result.y.fill_(0.0)
        ...
        torch.index_select(self._data.h_l, 0, self._data.idx_hl.long(), out=self._res.z_l)
        self._res.z_l *= -1.0
        torch.index_select(self._data.h_u, 0, self._data.idx_hu.long(), out=self._res.z_u)
        torch.index_select(self._data.x_l, 0, self._data.idx_xl.long(), out=self._res.z_bl)
        self._res.z_bl *= -1.0
        torch.index_select(self._data.x_u, 0, self._data.idx_xu.long(), out=self._res.z_bu)
        self._res.s_all[:] = 0.
        ...
        if self._data.num_hl + self._data.num_hu + self._data.num_xl + self._data.num_xu > 0:
            ...
            delta_s = -torch.min(self._result.s_all)
            delta_z = -torch.min(self._result.z_all)
            ...
            self._calculate_mu()
            torch.clamp(self._result.info.mu, 1e-10, None, out=self._result.info.mu)
            ...
            torch.sub(self._result.z_all, delta_z, out=self._result.s_all)
            torch.pow(self._result.s_all, 2, out=self._result.z_all)
            self._result.z_all += 4. * self._result.info.mu[0]
            torch.sqrt(self._result.z_all, out=self._result.z_all)
            self._result.z_all += self._result.s_all
            self._result.z_all /= 2.
            torch.sub(self._result.z_all, self._result.s_all, out=self._result.s_all)
    ...
    def _update_and_factorize_kkt(self) -> None:
        ...
        while self._result.info.factor_retires < self.settings.max_factor_retires:
            ...
            if factor_succeeded:
                ...
                self._result.info.reg_limit[:] = torch.minimum(10 * self._result.info.reg_limit, torch.tensor(self.settings.eps_abs, dtype=self._result.info.reg_limit.dtype, device=self._result.info.reg_limit.device))
    ...
    def _run_predictor_corrector(self):
        ...
        with nvtx.annotate("Solver::prepare_predictor_step"):
            torch.mul(self._result.s_all, self._result.z_all, out=self._res.s_all)
            self._res.s_all *= -1.
        ...
        with nvtx.annotate("Solver::prepare_corrector_step"):
            ...
            torch.mul(self._step.s_all, self._step.z_all, out=self._work_s)
        ...
        self._calculate_mu()
        torch.sub(self._mu_prev, self._result.info.mu, out=self._mu_rate)
        torch.div(self._mu_rate, self._mu_prev, out=self._mu_rate)
        torch.clamp_min(self._mu_rate, 0., out=self._mu_rate)
    ...
    def _calculate_step(self) -> None:
        ...
        self._work_s[:] = torch.where(self._step.s_all < 0, -self._result.s_all / self._step.s_all, 1.)
        self._result.info.primal_step[:] = torch.min(self._work_s)  # alpha_s
        self._result.info.primal_step *= self.settings.tau  # avoid getting too close to the boundary

        # alpha_z: step length for duals
        self._work_z[:] = torch.where(self._step.z_all < 0, -self._result.z_all / self._step.z_all, 1.)
        self._result.info.dual_step[:] = torch.min(self._work_z)  # alpha_z
    ...
    def _calculate_mu(self) -> None:
        self._result.info.mu[:] = torch.dot(self._result.s_all, self._result.z_all)
    ...
    def _calculate_sigma(self) -> None:
        self._result.info.sigma[:] = torch.dot(self._result.s_all + self._result.info.primal_step * self._step.s_all,
               self._result.z_all + self._result.info.dual_step * self._step.z_all)
        torch.div(self._result.info.sigma, self._result.info.mu, out=self._result.info.sigma)
        self._result.info.sigma /= self._data.num_hl + self._data.num_hu + self._data.num_xl + self._data.num_xu
        torch.clamp(self._result.info.sigma, 0., 1., out=self._result.info.sigma)
        torch.pow(self._result.info.sigma, 3., out=self._result.info.sigma)
    ...
    def _update_residuals_nr(self):
        ...
        torch.abs(self._res_nr.x, out=self._work_primals)
        self._work_primals *= self._unscale_dual_res_factor
        self._work_dual_res_norm[:] = torch.max(self._work_primals)
        ...
        if self._data.p > 0:
            torch.abs(self._res_nr.y, out=self._work_duals[:self._data.p])
            self._work_duals[:self._data.p] *= self._unscale_primal_res_eq_factor
            self._work_primal_rel_norm[:] = torch.max(self._work_duals[:self._data.p])
        else:
            self._work_primal_rel_norm.fill_(0.)

        self._work_z_1.fill_(0.)
        ...
        self._work_reduce[0:1] = torch.dot(self._res_nr.x, self._result.x)
        self._work_reduce[0:1] *= -0.5  # hold 0.5 * x^T P x
        self._work_reduce[1:2] = torch.dot(self._data.c, self._result.x) # hold c^T x

        torch.mul(self._work_reduce[0:1], 1., out=self._work_reduce[2:3])  # hold 0.5 * x^T P x
        self._work_reduce[3:4] = torch.dot(self._data.b, self._result.y) # hold b^T y
        self._work_reduce[4:5] = torch.dot(self._data.h_l[self._data.idx_hl], self._result.z_l)  # hold h_l^T z_l
        self._work_reduce[4:5] *= -1.  # hold - h_l^T z_l
        self._work_reduce[5:6] = torch.dot(self._data.h_u[self._data.idx_hu], self._result.z_u)  # hold h_u^T z_u
        self._work_reduce[6:7] = torch.dot(self._data.x_l[self._data.idx_xl], self._result.z_bl)
        self._work_reduce[6:7] *= -1.  # hold - x_l^T z_bl
        self._work_reduce[7:8] = torch.dot(self._data.x_u[self._data.idx_xu], self._result.z_bu)  # hold x_u^T z_bu

        self._result.info.primal_obj[:] = torch.sum(self._work_reduce[0:2])  # primal_obj = 0.5 x^T P x + c^T x
        self._result.info.dual_obj[:] = torch.sum(self._work_reduce[2:8])
        ...
        torch.sub(self._result.info.primal_obj, self._result.info.dual_obj, out=self._result.info.duality_gap)
        ...
        torch.abs(self._work_reduce, out=self._work_reduce)
        self._result.info.duality_gap_rel[:] = torch.max(self._work_reduce[0:8])
        torch.abs(self._result.info.duality_gap, out=self._result.info.duality_gap)
        torch.clamp_min(self._result.info.duality_gap_rel, 1., out=self._result.info.duality_gap_rel)
        torch.div(self._result.info.duality_gap, self._result.info.duality_gap_rel, out=self._result.info.duality_gap_rel)
        ...
        torch.index_select(G_x, 0, self._data.idx_hl.long(), out=self._res_nr.z_l)
        torch.sub(self._res_nr.z_l, self._result.s_l, out=self._res_nr.z_l)
        torch.sub(self._res_nr.z_l, self._data.h_l[self._data.idx_hl], out=self._res_nr.z_l)  # TODO: this creates a tmp array, optimize?

        # res_nr.z_u = -G*x - s_u + hu  =>  self._res_nr.z_u[:] = (-G_x[self._data.idx_hu] - self._result.s_u + self._data.h_u[self._data.idx_hu])
        torch.index_select(G_x, 0, self._data.idx_hu.long(), out=self._res_nr.z_u)
        torch.neg(self._res_nr.z_u, out=self._res_nr.z_u)
        torch.sub(self._res_nr.z_u, self._result.s_u, out=self._res_nr.z_u)
        torch.add(self._res_nr.z_u, self._data.h_u[self._data.idx_hu], out=self._res_nr.z_u)  # TODO: this creates a tmp array, optimize?

        # res_nr.z_bl = x_b_scaling*x - s_bl - xl
        torch.index_select(self._result.x, 0, self._data.idx_xl.long(), out=self._res_nr.z_bl)
        self._res_nr.z_bl *= self._data.x_b_scaling[self._data.idx_xl]
        torch.sub(self._res_nr.z_bl, self._result.s_bl, out=self._res_nr.z_bl)
        torch.sub(self._res_nr.z_bl, self._data.x_l[self._data.idx_xl], out=self._res_nr.z_bl)

        # res_nr.z_bu = -(x_b_scaling*x + s_bu - xu)
        torch.index_select(self._result.x, 0, self._data.idx_xu.long(), out=self._res_nr.z_bu)
        self._res_nr.z_bu *= self._data.x_b_scaling[self._data.idx_xu]
        torch.add(self._res_nr.z_bu, self._result.s_bu, out=self._res_nr.z_bu)
        torch.sub(self._res_nr.z_bu, self._data.x_u[self._data.idx_xu], out=self._res_nr.z_bu)
        torch.neg(self._res_nr.z_bu, out=self._res_nr.z_bu)
        ...
        if self._data.num_hu > 0:
            torch.index_select(G_x, 0, self._data.idx_hu.long(), out=self._work_z_1[:self._data.num_hu])
            torch.abs(self._work_z_1[:self._data.num_hu], out=self._work_z_1[:self._data.num_hu])
            self._work_z_1[:self._data.num_hu] *= self._unscale_primal_res_ineq_hu
            self._work_norm_temp[:] = torch.max(self._work_z_1[:self._data.num_hu])
            torch.maximum(self._work_primal_rel_norm, self._work_norm_temp, out=self._work_primal_rel_norm)

        if self._data.num_hl > 0:
            torch.index_select(G_x, 0, self._data.idx_hl.long(), out=self._work_z_1[:self._data.num_hl])
            torch.abs(self._work_z_1[:self._data.num_hl], out=self._work_z_1[:self._data.num_hl])
            self._work_z_1[:self._data.num_hl] *= self._unscale_primal_res_ineq_hl
            self._work_norm_temp[:] = torch.max(self._work_z_1[:self._data.num_hl])
            torch.maximum(self._work_primal_rel_norm, self._work_norm_temp, out=self._work_primal_rel_norm)

        if self._data.num_hu > 0:
            torch.abs(self._result.s_u, out=self._work_z_1[:self._data.num_hu])
            self._work_z_1[:self._data.num_hu] *= self._unscale_primal_res_ineq_hu
            self._work_norm_temp[:] = torch.max(self._work_z_1[:self._data.num_hu])
            torch.maximum(self._work_primal_rel_norm, self._work_norm_temp, out=self._work_primal_rel_norm)

        if self._data.num_hl > 0:
            torch.abs(self._result.s_l, out=self._work_z_1[:self._data.num_hl])
            self._work_z_1[:self._data.num_hl] *= self._unscale_primal_res_ineq_hl
            self._work_norm_temp[:] = torch.max(self._work_z_1[:self._data.num_hl])
            torch.maximum(self._work_primal_rel_norm, self._work_norm_temp, out=self._work_primal_rel_norm)

        if self._data.num_xu > 0:
            torch.abs(self._result.s_bu, out=self._work_z[:self._data.num_xu])
            self._work_z[:self._data.num_xu] *= self._unscale_primal_res_b_xu
            self._work_norm_temp[:] = torch.max(self._work_z[:self._data.num_xu])
            torch.maximum(self._work_primal_rel_norm, self._work_norm_temp, out=self._work_primal_rel_norm)

        if self._data.num_xl > 0:
            torch.abs(self._result.s_bl, out=self._work_z[:self._data.num_xl])
            self._work_z[:self._data.num_xl] *= self._unscale_primal_res_b_xl
            self._work_norm_temp[:] = torch.max(self._work_z[:self._data.num_xl])
            torch.maximum(self._work_primal_rel_norm, self._work_norm_temp, out=self._work_primal_rel_norm)

        torch.maximum(self._work_primal_rel_norm, self._constraints_rhs_inf_norm_unscaled, out=self._work_primal_rel_norm)
        # Store max(1, primal_rel_norm) for use by _update_residuals_r
        torch.clamp_min(self._work_primal_rel_norm, 1., out=self._work_primal_rel_norm)
        torch.div(self._result.info.primal_res, self._work_primal_rel_norm, out=self._result.info.primal_res_rel)
        ...
        torch.abs(self._data.c, out=self._work_primals)
        self._work_primals *= self._unscale_dual_res_factor
        self._work_norm_temp[:] = torch.max(self._work_primals)
        torch.maximum(self._work_dual_res_norm, self._work_norm_temp, out=self._work_dual_res_norm)
        ...
        torch.abs(self._res.x, out=self._work_primals)
        self._work_primals *= self._unscale_dual_res_factor
        self._work_norm_temp[:] = torch.max(self._work_primals)
        torch.maximum(self._work_dual_res_norm, self._work_norm_temp, out=self._work_dual_res_norm)

        # store max(1, dual_res_norm) for use by _update_residuals_r
        torch.clamp_min(self._work_dual_res_norm, 1., out=self._work_dual_res_norm)
        torch.div(self._result.info.dual_res, self._work_dual_res_norm, out=self._result.info.dual_res_rel)
    ...
    def _update_residuals_r(self):
        ...
        if USE_WARP_IMPLEMENTATION:
            ...
            torch.sub(self._result.x, self._prox_vars.x, out=self._res.x)
            self._res.x *= self._result.info.rho
            torch.sub(self._res_nr.x, self._res.x, out=self._res.x)
            torch.sub(self._prox_vars.duals_all, self._result.duals_all, out=self._res.duals_all)
            self._res.duals_all *= self._result.info.delta
            torch.sub(self._res_nr.duals_all, self._res.duals_all, out=self._res.duals_all)
        ...
        torch.div(self._result.info.primal_res, self._result.info.primal_res_rel, out=self._result.info.primal_res_reg_rel)
        self._result.info.primal_res_reg_rel[:] = torch.where(self._result.info.primal_res_rel > 0,
                self._result.info.primal_res_reg_rel, self._const_one)
        torch.div(self._result.info.primal_res_reg, self._result.info.primal_res_reg_rel, out=self._result.info.primal_res_reg_rel)
        ...
        torch.div(self._result.info.dual_res, self._result.info.dual_res_rel, out=self._result.info.dual_res_reg_rel)
        self._result.info.dual_res_reg_rel[:] = torch.where(self._result.info.dual_res_rel > 0,
                self._result.info.dual_res_reg_rel, self._const_one)
        torch.div(self._result.info.dual_res_reg, self._result.info.dual_res_reg_rel, out=self._result.info.dual_res_reg_rel)
    ...
    def _primal_res_nr(self) -> float:
        ...
        torch.abs(self._work_duals[:offset], out=self._work_duals[:offset])
        self._work_residual[()] = torch.max(self._work_duals[:offset])
    ...
    def _dual_res_nr(self) -> float:
        ...
        torch.abs(self._res_nr.x, out=self._work_primals)
        self._work_primals *= self._unscale_dual_res_factor
        self._work_residual[()] = torch.max(self._work_primals)
        return self._work_residual
    ...
    def _primal_prox_inf(self) -> float:
        torch.sub(self._result.duals_all, self._prox_vars.duals_all, out=self._work_duals)
        torch.abs(self._work_duals, out=self._work_duals)
        self._work_residual[()] = torch.max(self._work_duals)
        return self._work_residual

    @nvtx.annotate("Solver::_dual_prox_inf")
    def _dual_prox_inf(self) -> float:
        torch.sub(self._result.x, self._prox_vars.x, out=self._work_primals)
        torch.abs(self._work_primals, out=self._work_primals)
        self._work_residual[()] = torch.max(self._work_primals)
        return self._work_residual
    ...
    def _update_rho_delta_with_ineq(self) -> None:
        ...
        if self._result.info.dual_res < 0.95 * self._result.info.prev_dual_res or \
            ...
            self._result.info.rho[:] = torch.maximum(self._result.info.reg_limit, (1. - self._mu_rate) * self._result.info.rho)
        else:
            ...
            if self._result.info.iter < 5 or self._result.info.dual_prox_inf < self.settings.infeasibility_threshold:
                self._result.info.rho[:] = torch.maximum(self._result.info.reg_limit, (1. - 0.666 * self._mu_rate) * self._result.info.rho)
        ...
        if self._result.info.primal_res < 0.95 * self._result.info.prev_primal_res or \
            ...
            self._prox_vars.duals_all[:] = self._result.duals_all
            self._result.info.delta[:] = torch.maximum(self._result.info.reg_limit, (1. - self._mu_rate) * self._result.info.delta)
        else:
            ...
            if self._result.info.iter < 5 or self._result.info.primal_prox_inf < self.settings.infeasibility_threshold:
                self._result.info.delta[:] = torch.maximum(self._result.info.reg_limit, (1. - 0.666 * self._mu_rate) * self._result.info.delta)
    ...
    def _update_rho_delta_without_ineq(self) -> None:
        ...
        if self._result.info.dual_res < 0.95 * self._result.info.prev_dual_res or \
            ...
            self._prox_vars.x[:] = self._result.x
            self._result.info.rho[:] = torch.maximum(self._result.info.reg_limit, 0.1 * self._result.info.rho)
        else:
            self._result.info.no_primal_update += 1
            if self._result.info.iter < 5 or self._result.info.dual_prox_inf < self.settings.infeasibility_threshold:
                self._result.info.rho[:] = torch.maximum(self._result.info.reg_limit, 0.5 * self._result.info.rho)
        ...
        if self._result.info.primal_res < 0.95 * self._result.info.prev_primal_res or \
            ...
            self._result.info.delta[:] = torch.maximum(self._result.info.reg_limit, 0.1 * self._result.info.delta)
        else:
            self._result.info.no_dual_update += 1
            if self._result.info.iter < 5 or self._result.info.primal_prox_inf < self.settings.infeasibility_threshold:
                self._result.info.delta[:] = torch.maximum(self._result.info.reg_limit, 0.5 * self._result.info.delta)
