...
class Variables:
    ...
    def init(self, data: Data):
        ...
        self._primal_buffer = torch.empty(data.n + self.num_ineq, dtype=torch.float64, device='cuda')
        ...
        self._dual_buffer = torch.empty(data.p + self.num_ineq, dtype=torch.float64, device='cuda')
    ...
    def all_finite(self) -> bool:
        return (torch.isfinite(self._primal_buffer).all() and
                torch.isfinite(self._dual_buffer).all())
    ...
    def buffer_ptr(self) -> tuple:
        ...
        """
        return (
            self._primal_buffer.data_ptr(),
            self._dual_buffer.data_ptr(),
        )

    def allclose(self, other: 'Variables', rtol: float = 1e-8, atol: float = 1e-8) -> bool:
        return (torch.allclose(self._primal_buffer, other._primal_buffer, rtol=rtol, atol=atol) and
                torch.allclose(self._dual_buffer, other._dual_buffer, rtol=rtol, atol=atol))
    ...
    def to_array(self) -> torch.Tensor:
        return torch.cat((self._primal_buffer, self._dual_buffer))

    def set_random(self):
        """Testing purpose only: set all variables to random values."""
        torch.manual_seed(0)
        self._x[:] = torch.randn(*self._x.shape, dtype=torch.float64, device='cuda')
        self._y[:] = torch.randn(*self._y.shape, dtype=torch.float64, device='cuda')
        self._z_u[:] = torch.rand(*self._z_u.shape, dtype=torch.float64, device='cuda') + 1.0  # ensure positivity
        self._z_l[:] = torch.rand(*self._z_l.shape, dtype=torch.float64, device='cuda') + 1.0
        self._z_bu[:] = torch.rand(*self._z_bu.shape, dtype=torch.float64, device='cuda') + 1.0
        self._z_bl[:] = torch.rand(*self._z_bl.shape, dtype=torch.float64, device='cuda') + 1.0
        self._s_u[:] = torch.rand(*self._s_u.shape, dtype=torch.float64, device='cuda') + 1.0
        self._s_l[:] = torch.rand(*self._s_l.shape, dtype=torch.float64, device='cuda') + 1.0
        self._s_bu[:] = torch.rand(*self._s_bu.shape, dtype=torch.float64, device='cuda') + 1.0
        self._s_bl[:] = torch.rand(*self._s_bl.shape, dtype=torch.float64, device='cuda') + 1.0
...
class Info:
    ...
    def init(self):
        self._buffer = torch.zeros(len(InfoIdx), dtype=torch.float64, device='cuda')  # one contiguous GPU buffer for all scalar fields
    ...
    def to_host(self, info_host: 'InfoHost'):
        info_host._buffer[:] = self._buffer.cpu().numpy()
