"""
Unit tests for KKTSystem with dense backend: condensed factorize-solve round-trip.

Mirrors test_sparse_kkt.py but uses DenseData and DenseKKTSolver.
"""
import pytest
import numpy as np
import cupy as cp
...
def make_random_dense_qp(n=20, p=8, m=9, density=0.3, seed=42) -> DenseData:
    """Generate a random strongly-convex dense QP."""
    rng = np.random.default_rng(seed)

    # P: symmetric positive definite
    M = rng.standard_normal((n, n)) * density
    P = M @ M.T + n * np.eye(n)
    P = (P + P.T) / 2
    ...
    A = rng.standard_normal((p, n)) if p > 0 else np.zeros((0, n))
    b = rng.standard_normal(p)

    # G: inequality constraints
    G = rng.standard_normal((m, n)) if m > 0 else np.zeros((0, n))
...
class TestDenseKKTSystemCondensedSolve:
    """Test that solving the condensed KKT system (dense backend) and
    multiplying back gives the original RHS."""
    ...
    def test_condensed_factorize_solve(self, n, p, m):
        ...
        mul_condensed_kkt."""
        data = make_random_dense_qp(n, p, m)
