...
class Variables:
    ...
    def __init__(self, n: int, p: int, m: int):
        ...
        self.x = jnp.zeros(n)        # Primal variables
        self.y = jnp.zeros(p)        # Dual variables for equality constraints
        self.z_u = jnp.ones(m)      # Dual variables for inequality constraints (upper)
        self.z_l = jnp.ones(m)      # Dual variables for inequality constraints (lower)
        self.z_bl = jnp.ones(n)     # Dual variables for bound constraints (lower)
        self.z_bu = jnp.ones(n)     # Dual variables for bound constraints (upper)
        self.s_u = jnp.ones(m)      # Slack variables for inequality constraints (upper)
        self.s_l = jnp.ones(m)      # Slack variables for inequality constraints (lower)
        self.s_bl = jnp.ones(n)     # Slack variables for bound constraints (lower)
        self.s_bu = jnp.ones(n)     # Slack variables for bound constraints (upper)
    ...
    def all_finite(self) -> bool:
        return (jnp.isfinite(self.x).all() and
                jnp.isfinite(self.y).all() and
                jnp.isfinite(self.z_u).all() and
                jnp.isfinite(self.z_l).all() and
                jnp.isfinite(self.z_bl).all() and
                jnp.isfinite(self.z_bu).all() and
                jnp.isfinite(self.s_u).all() and
                jnp.isfinite(self.s_l).all() and
                jnp.isfinite(self.s_bl).all() and
                jnp.isfinite(self.s_bu).all())
    
    def allclose(self, other: 'Variables', rtol: float = 1e-8, atol: float = 1e-8) -> bool:
        return (jnp.allclose(self.x, other.x, rtol=rtol, atol=atol) and
                jnp.allclose(self.y, other.y, rtol=rtol, atol=atol) and
                jnp.allclose(self.z_u, other.z_u, rtol=rtol, atol=atol) and
                jnp.allclose(self.z_l, other.z_l, rtol=rtol, atol=atol) and
                jnp.allclose(self.z_bl, other.z_bl, rtol=rtol, atol=atol) and
                jnp.allclose(self.z_bu, other.z_bu, rtol=rtol, atol=atol) and
                jnp.allclose(self.s_u, other.s_u, rtol=rtol, atol=atol) and
                jnp.allclose(self.s_l, other.s_l, rtol=rtol, atol=atol) and
                jnp.allclose(self.s_bl, other.s_bl, rtol=rtol, atol=atol) and
                jnp.allclose(self.s_bu, other.s_bu, rtol=rtol, atol=atol))
    ...
    def to_array(self) -> jnp.ndarray:
        return jnp.concatenate((self.x, self.y, self.z_u, self.z_l, self.z_bu, self.z_bl, self.s_u, self.s_l, self.s_bu, self.s_bl))
    
    def set_random(self):
        """Testing purpose only: set all variables to random values."""
        jnp.random.seed(0)
        self.x = jnp.array(jnp.random.randn(*self.x.shape))
        self.y = jnp.array(jnp.random.randn(*self.y.shape))
        self.z_u = jnp.array(jnp.random.rand(*self.z_u.shape)) + 1.0  # ensure positivity
        self.z_l = jnp.array(jnp.random.rand(*self.z_l.shape)) + 1.0
        self.z_bu = jnp.array(jnp.random.rand(*self.z_bu.shape)) + 1.0
        self.z_bl = jnp.array(jnp.random.rand(*self.z_bl.shape)) + 1.0
        self.s_u = jnp.array(jnp.random.rand(*self.s_u.shape)) + 1.0
        self.s_l = jnp.array(jnp.random.rand(*self.s_l.shape)) + 1.0
        self.s_bu = jnp.array(jnp.random.rand(*self.s_bu.shape)) + 1.0
        self.s_bl = jnp.array(jnp.random.rand(*self.s_bl.shape)) + 1.0
