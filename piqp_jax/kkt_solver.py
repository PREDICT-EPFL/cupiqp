...
class KKTSolverBase(ABC):
    ...
    def __init__(self, data: Data):
        self._x_reg = jnp.nan * jnp.ones(data.n)
        self._z_reg_inv = jnp.nan * jnp.ones(data.m)
    ...
    def eval_P_x(self, data: Data, alpha: float, x: Vector) -> Vector:
        ...
        return _eval_P_x_jit(alpha, data.P, x)
    ...
    def eval_A_xn_and_AT_xt(self, data: Data, alpha_n: float, alpha_t: float, xn: Vector, xt: Vector):
        ...
        """
        return _eval_A_AT_jit(alpha_n, alpha_t, data.A, xn, xt)
...
class DenseKKTSolver(KKTSolverBase):
    ...
    def __init__(self, data: Data):
        ...
        self._delta = jnp.nan
        self._x_reg = jnp.nan * jnp.ones(data.n)  # used to store the diag(P) + reg_x, i.e., [... P_ii + reg_x_i ...]
        self._z_reg_inv = jnp.nan * jnp.ones(data.m)  # used to store the inverse of diag(W+delta*I), i.e., [... (s_i/z_i + delta)^-1 ...]
        
        self._kkt_mat = jnp.zeros((data.n, data.n))  # Placeholder for KKT matrix

        self._AtA = data.A.T @ data.A if data.p > 0 else jnp.zeros((0, 0))
    ...
    def solve(self, data: Data, rhs_x, rhs_y, rhs_z):
        ...
        lhs_x_tmp = jnp.array(rhs_x).flatten()
        if data.p > 0:
            lhs_x_tmp += (data.A.T @ rhs_y) / self._delta
        if data.m > 0:
            lhs_x_tmp += data.G.T @ (self._z_reg_inv * rhs_z)
        # Solve L * L^T * dx = effective_rhs
        y = solve_triangular(self._kkt_mat, lhs_x_tmp, lower=True)
        lhs_x = solve_triangular(self._kkt_mat.T, y, lower=False)

        # dy = 1/delta * (A * dx - r_y)
        lhs_y = 1/self._delta * (data.A @ lhs_x - rhs_y) if data.p > 0 else jnp.array([])

        # dz = (W+delta*I)^-1 * (G * dx - r_z)
        lhs_z = self._z_reg_inv * (data.G @ lhs_x - rhs_z) if data.m > 0 else jnp.array([])
...
class SparseKKTSolver(KKTSolverBase):
    ...
    def __init__(self, data: Data):
        ...
        self._kkt_mat = jnp.zeros((data.n + data.p + data.m, data.n + data.p + data.m))  # Placeholder for KKT matrix
    ...
    def update_scalings_and_factor(self, data: Data, delta: float, x_reg: Vector, z_reg: Vector) -> bool:
        ...
        self._kkt_mat[:n, :n] = data.P + jnp.diag(self._x_reg)
        self._kkt_mat[:n, n:n+p] = data.A.T
        self._kkt_mat[:n, n+p:] = data.G.T
        self._kkt_mat[n:n+p, :n] = data.A
        self._kkt_mat[n:n+p, n:n+p] = -self._delta * jnp.eye(p)
        self._kkt_mat[n+p:, :n] = data.G
        self._kkt_mat[n+p:, n+p:] = -jnp.diag(1.0 / self._z_reg_inv)
    ...
    def solve(self, data: Data, rhs_x, rhs_y, rhs_z):
        rhs = jnp.concatenate([rhs_x, rhs_y, rhs_z])
        lhs = jnp.linalg.solve(self._kkt_mat, rhs)
